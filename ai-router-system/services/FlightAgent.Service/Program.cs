using AiRouter.FlightAgent.Service.Agent;
using AiRouter.FlightAgent.Service.Tools;
...
builder.Services.AddScoped<IAgentTool, FlightSearchTool>();
...
app.MapPost("/agent/query", async (AgentQueryRequest request, AgentService service, CancellationToken cancellationToken) =>
{
    ...
    return Results.Ok(new AgentQueryResponse("FlightAgent.Service", content));
});
